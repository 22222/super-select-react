...
`useOptionSource` keeps the same source until its dependencies change.  When the fetch logic reads a prop or state value, pass a
dependency list as the second argument, like `useMemo`:
...
Skip the hook and call createOptionSource directly when the source lives outside a component or when you want to control
memoization yourself with `useMemo`:
