...
A source can also work directly with something like a local array:
...
The `useOptionSource` keeps the same source until its dependencies change.  When the fetch logic reads a prop or state value, pass a
dependency list as the second argument, just like in the `useMemo` hook:
...
You can also skip the hook and use the `createOptionSource` function directly.  For example, if you need to create the option source 
outside of a component or want to `useMemo` directly, like:
...
Option sources cache the first unfiltered page and options returned while resolving selected values. This avoids
repeating common requests and allows selected values to reuse labels that have already been loaded. Search results and
additional pages are still requested from the source when needed.
